...
header_prolog(TreeState *state)
{
    ...
    fprintf(state->file, "/*\n * DO NOT EDIT.  THIS FILE IS GENERATED FROM"
            " %s.idl\n */\n", state->basename);
    fprintf(state->file,
            "\nmodule mozilla.xpcom.%s;\n",
            define);

	if (strcmp("nsrootidl", state->basename) != 0)
	{
    	fprintf(state->file,
    ...
    if (state->base_includes != NULL) {
        ...
        for (i = 0; i < len; i++) {
			...
			char* className = (char *)g_slist_nth_data(state->base_includes, i);
            if (strcmp(className, "domstubs") == 0) {
                ...
                /* fputs("mixin(import(\"mozilla/xpcom/domstubs.d\"));\n", state->file); */
			}
...
#define FAIL    do {ok = FALSE; goto out;} while(0)
    ...
    if (iid) {
		fputs("  static const char[] IID_STR = ", state->file);
		write_classname_iid_define(state->file, className);
		fputs("_STR;\n", state->file);

		fputs("  static const nsIID IID = ", state->file);
        write_classname_iid_define(state->file, className);
        fputs(";\n\n", state->file);
    }
      ...
      case IDLN_IDENT:
        if (UP_IS_NATIVE(type_tree)) {
            if (IDL_tree_property_get(type_tree, "domstring") ||
                ...
                char* type = IDL_NATIVE(IDL_NODE_UP(type_tree)).user_type;
                int len = strlen(type);
                int i;
...
forward_dcl(TreeState *state)
{
	...
	if (strcmp(className, "nsIMsgDBViewCommandUpdater") == 0)
	    ...
	    fprintf(state->file, "public import mozilla.xpcom.%s; /* forward declaration */\n\n", className);
...
codefrag(TreeState *state)
{
    ...
    if (strcmp(desc, "D") && /* libIDL bug? */ strcmp(desc, "D\r")) {
        XPIDL_WARNING((state->tree, IDL_WARNING1,
                       "ignoring '%%{%s' escape. "
