...
module mozilla.dxpcom.nsIDOMDOMConfigurationD;
...
/* starting wrapper class:    nsIDOMDOMConfiguration */
/**
 ...
 */
class nsIDOMDOMConfigurationD : public nsISupportsD {

  static const nsIID IID = NS_IDOMDOMCONFIGURATION_IID;


  alias nsIDOMDOMConfiguration InnerType;

  this(nsIDOMDOMConfiguration intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMDOMConfiguration opCast() {
    return inner;
  }

  void opAssign(nsIDOMDOMConfiguration value) {
    inner = value;
  }

  /* void setParameter (in DOMString name, in nsIVariant value)  raises (DOMException); */
  void SetParameter(wchar[] name, nsIVariantD value){
    scope auto _name = new AString(name);
    nsresult __result = inner.SetParameter(cast(nsAString*)_name, value ? cast(nsIVariant)value : null);
    CheckException(__result);
  }

  /* nsIVariant getParameter (in DOMString name)  raises (DOMException); */
  nsIVariantD  GetParameter(wchar[] name){
    scope auto _name = new AString(name);
    nsIVariant _retval;
    nsresult __result = inner.GetParameter(cast(nsAString*)_name, &_retval);
    CheckException(__result);
    return new nsIVariantD(_retval);
  }

  /* boolean canSetParameter (in DOMString name, in nsIVariant value); */
  PRBool CanSetParameter(wchar[] name, nsIVariantD value){
    scope auto _name = new AString(name);
    PRBool _retval;
    nsresult __result = inner.CanSetParameter(cast(nsAString*)_name, value ? cast(nsIVariant)value : null, &_retval);
    CheckException(__result);
    return _retval;
  }

private:
  nsIDOMDOMConfiguration inner;
