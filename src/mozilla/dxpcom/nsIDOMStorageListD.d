...
module mozilla.dxpcom.nsIDOMStorageListD;
...
/* starting wrapper class:    nsIDOMStorageList */
class nsIDOMStorageListD : public nsISupportsD {

  static const nsIID IID = NS_IDOMSTORAGELIST_IID;


  alias nsIDOMStorageList InnerType;

  this(nsIDOMStorageList intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMStorageList opCast() {
    return inner;
  }

  void opAssign(nsIDOMStorageList value) {
    inner = value;
  }
  ...
  nsIDOMStorageD  NamedItem(wchar[] domain){
    scope auto _domain = new AString(domain);
    nsIDOMStorage _retval;
    nsresult __result = inner.NamedItem(cast(nsAString*)_domain, &_retval);
    CheckException(__result);
    return new nsIDOMStorageD(_retval);
  }

private:
  nsIDOMStorageList inner;
