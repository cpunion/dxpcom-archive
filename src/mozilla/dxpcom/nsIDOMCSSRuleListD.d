...
module mozilla.dxpcom.nsIDOMCSSRuleListD;
...
/* starting wrapper class:    nsIDOMCSSRuleList */
class nsIDOMCSSRuleListD : public nsISupportsD {

  static const nsIID IID = NS_IDOMCSSRULELIST_IID;


  alias nsIDOMCSSRuleList InnerType;

  this(nsIDOMCSSRuleList intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMCSSRuleList opCast() {
    return inner;
  }

  void opAssign(nsIDOMCSSRuleList value) {
    inner = value;
  }
 ...
 */
  ...
  nsIDOMCSSRuleD  Item(PRUint32 index){
    nsIDOMCSSRule _retval;
    nsresult __result = inner.Item(index, &_retval);
    CheckException(__result);
    return new nsIDOMCSSRuleD(_retval);
  }

private:
  nsIDOMCSSRuleList inner;
