...
module mozilla.dxpcom.nsIDOMXPathExpressionD;
...
/* starting wrapper class:    nsIDOMXPathExpression */
class nsIDOMXPathExpressionD : public nsISupportsD {

  static const nsIID IID = NS_IDOMXPATHEXPRESSION_IID;


  alias nsIDOMXPathExpression InnerType;

  this(nsIDOMXPathExpression intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMXPathExpression opCast() {
    return inner;
  }

  void opAssign(nsIDOMXPathExpression value) {
    inner = value;
  }
  ...
  nsISupportsD  Evaluate(nsIDOMNodeD contextNode, PRUint16 type, nsISupportsD result){
    nsISupports _retval;
    nsresult __result = inner.Evaluate(contextNode ? cast(nsIDOMNode)contextNode : null, type, result ? cast(nsISupports)result : null, &_retval);
    CheckException(__result);
    return new nsISupportsD(_retval);
  }

private:
  nsIDOMXPathExpression inner;
