...
module mozilla.dxpcom.nsIDOMSVGPointD;
...
/* starting wrapper class:    nsIDOMSVGPoint */
class nsIDOMSVGPointD : public nsISupportsD {

  static const nsIID IID = NS_IDOMSVGPOINT_IID;


  alias nsIDOMSVGPoint InnerType;

  this(nsIDOMSVGPoint intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMSVGPoint opCast() {
    return inner;
  }

  void opAssign(nsIDOMSVGPoint value) {
    inner = value;
  }
  ...
  nsIDOMSVGPointD  MatrixTransform(nsIDOMSVGMatrixD matrix){
    nsIDOMSVGPoint _retval;
    nsresult __result = inner.MatrixTransform(matrix ? cast(nsIDOMSVGMatrix)matrix : null, &_retval);
    CheckException(__result);
    return new nsIDOMSVGPointD(_retval);
  }

private:
  nsIDOMSVGPoint inner;
