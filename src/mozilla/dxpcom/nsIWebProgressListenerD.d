...
module mozilla.dxpcom.nsIWebProgressListenerD;
...
/* starting wrapper class:    nsIWebProgressListener */
/**
 ...
 * @status FROZEN
 */
class nsIWebProgressListenerD : public nsISupportsD {

  static const nsIID IID = NS_IWEBPROGRESSLISTENER_IID;


  alias nsIWebProgressListener InnerType;

  this(nsIWebProgressListener intr){
    super(intr);
    this.inner = intr;
  }

  nsIWebProgressListener opCast() {
    return inner;
  }

  void opAssign(nsIWebProgressListener value) {
    inner = value;
  }
  ...
  void OnStateChange(nsIWebProgressD aWebProgress, nsIRequestD aRequest, PRUint32 aStateFlags, nsresult aStatus){
    nsresult __result = inner.OnStateChange(aWebProgress ? cast(nsIWebProgress)aWebProgress : null, aRequest ? cast(nsIRequest)aRequest : null, aStateFlags, aStatus);
  ...
  void OnProgressChange(nsIWebProgressD aWebProgress, nsIRequestD aRequest, PRInt32 aCurSelfProgress, PRInt32 aMaxSelfProgress, PRInt32 aCurTotalProgress, PRInt32 aMaxTotalProgress){
    nsresult __result = inner.OnProgressChange(aWebProgress ? cast(nsIWebProgress)aWebProgress : null, aRequest ? cast(nsIRequest)aRequest : null, aCurSelfProgress, aMaxSelfProgress, aCurTotalProgress, aMaxTotalProgress);
    CheckException(__result);
  }
  ...
  void OnLocationChange(nsIWebProgressD aWebProgress, nsIRequestD aRequest, nsIURID aLocation){
    nsresult __result = inner.OnLocationChange(aWebProgress ? cast(nsIWebProgress)aWebProgress : null, aRequest ? cast(nsIRequest)aRequest : null, aLocation ? cast(nsIURI)aLocation : null);
    CheckException(__result);
  }
  ...
  void OnStatusChange(nsIWebProgressD aWebProgress, nsIRequestD aRequest, nsresult aStatus, PRUnichar*aMessage){
    nsresult __result = inner.OnStatusChange(aWebProgress ? cast(nsIWebProgress)aWebProgress : null, aRequest ? cast(nsIRequest)aRequest : null, aStatus, aMessage);
    CheckException(__result);
  }
  ...
  void OnSecurityChange(nsIWebProgressD aWebProgress, nsIRequestD aRequest, PRUint32 aState){
    nsresult __result = inner.OnSecurityChange(aWebProgress ? cast(nsIWebProgress)aWebProgress : null, aRequest ? cast(nsIRequest)aRequest : null, aState);
    CheckException(__result);
  }

private:
  nsIWebProgressListener inner;
