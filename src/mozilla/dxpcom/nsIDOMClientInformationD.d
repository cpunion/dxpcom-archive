...
module mozilla.dxpcom.nsIDOMClientInformationD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsIDOMClientInformation;


public import mozilla.dxpcom.nsIDOMClientInformationD;
...
/* starting wrapper class:    nsIDOMClientInformation */
class nsIDOMClientInformationD : public nsISupportsD {

  static const nsIID IID = NS_IDOMCLIENTINFORMATION_IID;


  alias nsIDOMClientInformation InnerType;

  this(nsIDOMClientInformation intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMClientInformation opCast() {
    return inner;
  }

  void opAssign(nsIDOMClientInformation value) {
    inner = value;
  }
  ...
  void RegisterContentHandler(wchar[] mimeType, wchar[] uri, wchar[] title){
    scope auto _mimeType = new AString(mimeType);
    scope auto _uri = new AString(uri);
    scope auto _title = new AString(title);
    nsresult __result = inner.RegisterContentHandler(cast(nsAString*)_mimeType, cast(nsAString*)_uri, cast(nsAString*)_title);
    CheckException(__result);
  }

  /* void registerProtocolHandler (in DOMString protocol, in DOMString uri, in DOMString title); */
  void RegisterProtocolHandler(wchar[] protocol, wchar[] uri, wchar[] title){
    scope auto _protocol = new AString(protocol);
    scope auto _uri = new AString(uri);
    scope auto _title = new AString(title);
    nsresult __result = inner.RegisterProtocolHandler(cast(nsAString*)_protocol, cast(nsAString*)_uri, cast(nsAString*)_title);
    CheckException(__result);
  }

private:
  nsIDOMClientInformation inner;
