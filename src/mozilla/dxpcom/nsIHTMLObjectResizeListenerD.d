...
module mozilla.dxpcom.nsIHTMLObjectResizeListenerD;
...
/* starting wrapper class:    nsIHTMLObjectResizeListener */
class nsIHTMLObjectResizeListenerD : public nsISupportsD {

  static const nsIID IID = NS_IHTMLOBJECTRESIZELISTENER_IID;


  alias nsIHTMLObjectResizeListener InnerType;

  this(nsIHTMLObjectResizeListener intr){
    super(intr);
    this.inner = intr;
  }

  nsIHTMLObjectResizeListener opCast() {
    return inner;
  }

  void opAssign(nsIHTMLObjectResizeListener value) {
    inner = value;
  }
  ...
  void OnStartResizing(nsIDOMElementD aElement){
    nsresult __result = inner.OnStartResizing(aElement ? cast(nsIDOMElement)aElement : null);
  ...
  void OnEndResizing(nsIDOMElementD aElement, PRInt32 aOldWidth, PRInt32 aOldHeight, PRInt32 aNewWidth, PRInt32 aNewHeight){
    nsresult __result = inner.OnEndResizing(aElement ? cast(nsIDOMElement)aElement : null, aOldWidth, aOldHeight, aNewWidth, aNewHeight);
    CheckException(__result);
  }

private:
  nsIHTMLObjectResizeListener inner;
