...
module mozilla.dxpcom.nsIDOMMimeTypeD;
...
/* starting wrapper class:    nsIDOMMimeType */
class nsIDOMMimeTypeD : public nsISupportsD {

  static const nsIID IID = NS_IDOMMIMETYPE_IID;


  alias nsIDOMMimeType InnerType;

  this(nsIDOMMimeType intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMMimeType opCast() {
    return inner;
  }

  void opAssign(nsIDOMMimeType value) {
    inner = value;
  }
  ...
  nsIDOMPluginD  EnabledPlugin(){
    nsIDOMPlugin value;
    nsresult __result = inner.GetEnabledPlugin(&value);
    CheckException(__result);
    return new nsIDOMPluginD(value);
  }

  /* readonly attribute DOMString suffixes; */
  wchar[] Suffixes(){
    scope auto value = new AString();
    nsresult __result = inner.GetSuffixes(cast(nsAString*)value);
    CheckException(__result);
    return value.GetString();
  }
