...
module mozilla.dxpcom.nsIEditingSessionD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsIEditingSession;


public import mozilla.dxpcom.nsIEditingSessionD;
...
/* starting wrapper class:    nsIEditingSession */
class nsIEditingSessionD : public nsISupportsD {

  static const nsIID IID = NS_IEDITINGSESSION_IID;


  alias nsIEditingSession InnerType;

  this(nsIEditingSession intr){
    super(intr);
    this.inner = intr;
  }

  nsIEditingSession opCast() {
    return inner;
  }

  void opAssign(nsIEditingSession value) {
    inner = value;
  }
  ...
  PRUint32 EditorStatus(){
    PRUint32 value;
    nsresult __result = inner.GetEditorStatus(&value);
  ...
  void MakeWindowEditable(nsIDOMWindowD window, char*aEditorType, PRBool doAfterUriLoad){
    nsresult __result = inner.MakeWindowEditable(window ? cast(nsIDOMWindow)window : null, aEditorType, doAfterUriLoad);
    CheckException(__result);
  }
  ...
  PRBool WindowIsEditable(nsIDOMWindowD window){
    PRBool _retval;
    nsresult __result = inner.WindowIsEditable(window ? cast(nsIDOMWindow)window : null, &_retval);
    CheckException(__result);
    return _retval;
  }
  ...
  nsIEditorD  GetEditorForWindow(nsIDOMWindowD window){
    nsIEditor _retval;
    nsresult __result = inner.GetEditorForWindow(window ? cast(nsIDOMWindow)window : null, &_retval);
    CheckException(__result);
    return new nsIEditorD(_retval);
  }
  ...
  void SetupEditorOnWindow(nsIDOMWindowD window){
    nsresult __result = inner.SetupEditorOnWindow(window ? cast(nsIDOMWindow)window : null);
    CheckException(__result);
  }
  ...
  void TearDownEditorOnWindow(nsIDOMWindowD window){
    nsresult __result = inner.TearDownEditorOnWindow(window ? cast(nsIDOMWindow)window : null);
    CheckException(__result);
  }

  /* void setEditorOnControllers (in nsIDOMWindow aWindow, in nsIEditor aEditor); */
  void SetEditorOnControllers(nsIDOMWindowD aWindow, nsIEditorD aEditor){
    nsresult __result = inner.SetEditorOnControllers(aWindow ? cast(nsIDOMWindow)aWindow : null, aEditor ? cast(nsIEditor)aEditor : null);
    CheckException(__result);
  }

private:
  nsIEditingSession inner;
