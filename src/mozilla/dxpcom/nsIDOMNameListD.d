...
module mozilla.dxpcom.nsIDOMNameListD;
...
/* starting wrapper class:    nsIDOMNameList */
class nsIDOMNameListD : public nsISupportsD {

  static const nsIID IID = NS_IDOMNAMELIST_IID;


  alias nsIDOMNameList InnerType;

  this(nsIDOMNameList intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMNameList opCast() {
    return inner;
  }

  void opAssign(nsIDOMNameList value) {
    inner = value;
  }

  /* DOMString getName (in unsigned long index)  raises (DOMException); */
  wchar[] GetName(PRUint32 index){
    scope auto _retval = new AString;
    nsresult __result = inner.GetName(index, cast(nsAString*)_retval);
    CheckException(__result);
    return _retval.GetString();
  }

  /* DOMString getNamespaceURI (in unsigned long index)  raises (DOMException); */
  wchar[] GetNamespaceURI(PRUint32 index){
    scope auto _retval = new AString;
    nsresult __result = inner.GetNamespaceURI(index, cast(nsAString*)_retval);
    CheckException(__result);
    return _retval.GetString();
  }
  ...
  PRBool Contains(wchar[] str){
    scope auto _str = new AString(str);
    PRBool _retval;
    nsresult __result = inner.Contains(cast(nsAString*)_str, &_retval);
    CheckException(__result);
    return _retval;
  }

  /* boolean containsNS (in DOMString namespaceURI, in DOMString name); */
  PRBool ContainsNS(wchar[] namespaceURI, wchar[] name){
    scope auto _namespaceURI = new AString(namespaceURI);
    scope auto _name = new AString(name);
    PRBool _retval;
    nsresult __result = inner.ContainsNS(cast(nsAString*)_namespaceURI, cast(nsAString*)_name, &_retval);
    CheckException(__result);
    return _retval;
  }

private:
  nsIDOMNameList inner;
