...
module mozilla.dxpcom.nsIDOMPluginD;
...
/* starting wrapper class:    nsIDOMPlugin */
class nsIDOMPluginD : public nsISupportsD {

  static const nsIID IID = NS_IDOMPLUGIN_IID;


  alias nsIDOMPlugin InnerType;

  this(nsIDOMPlugin intr){
    super(intr);
    this.inner = intr;
  }
  ...
  void opAssign(nsIDOMPlugin value) {
    inner = value;
  }
  ...
  wchar[] Filename(){
    scope auto value = new AString();
    nsresult __result = inner.GetFilename(cast(nsAString*)value);
    CheckException(__result);
    return value.GetString();
  }
