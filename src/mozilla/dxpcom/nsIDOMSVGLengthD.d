...
module mozilla.dxpcom.nsIDOMSVGLengthD;
...
/* starting wrapper class:    nsIDOMSVGLength */
class nsIDOMSVGLengthD : public nsISupportsD {

  static const nsIID IID = NS_IDOMSVGLENGTH_IID;


  alias nsIDOMSVGLength InnerType;

  this(nsIDOMSVGLength intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMSVGLength opCast() {
    return inner;
  }

  void opAssign(nsIDOMSVGLength value) {
    inner = value;
  }
  ...
  PRUint16 UnitType(){
    PRUint16 value;
    nsresult __result = inner.GetUnitType(&value);
  ...
  float ValueInSpecifiedUnits(){
    float value;
    nsresult __result = inner.GetValueInSpecifiedUnits(&value);
    CheckException(__result);
    return value;
  }
  void ValueInSpecifiedUnits(float aValueInSpecifiedUnits){
    nsresult __result = inner.SetValueInSpecifiedUnits(aValueInSpecifiedUnits);
    CheckException(__result);
  }
  ...
  wchar[] ValueAsString(){
    ...
    nsresult __result = inner.GetValueAsString(cast(nsAString*)value);
    CheckException(__result);
    return value.GetString();
  }
  void ValueAsString(wchar[] aValueAsString){
    scope auto value = new AString(aValueAsString);
    nsresult __result = inner.SetValueAsString(cast(nsAString*)value);
    CheckException(__result);
  }

  /* void newValueSpecifiedUnits (in unsigned short unitType, in float valueInSpecifiedUnits); */
  void NewValueSpecifiedUnits(PRUint16 unitType, float valueInSpecifiedUnits){
    nsresult __result = inner.NewValueSpecifiedUnits(unitType, valueInSpecifiedUnits);
    CheckException(__result);
  }

  /* void convertToSpecifiedUnits (in unsigned short unitType); */
  void ConvertToSpecifiedUnits(PRUint16 unitType){
    nsresult __result = inner.ConvertToSpecifiedUnits(unitType);
    CheckException(__result);
  }

  /* float getTransformedValue (in nsIDOMSVGMatrix matrix); */
  float GetTransformedValue(nsIDOMSVGMatrixD matrix){
    float _retval;
    nsresult __result = inner.GetTransformedValue(matrix ? cast(nsIDOMSVGMatrix)matrix : null, &_retval);
    CheckException(__result);
    return _retval;
  }

private:
  nsIDOMSVGLength inner;
