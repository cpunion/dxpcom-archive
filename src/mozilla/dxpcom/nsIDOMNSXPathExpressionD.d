...
module mozilla.dxpcom.nsIDOMNSXPathExpressionD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsIDOMNSXPathExpression;


public import mozilla.dxpcom.nsIDOMNSXPathExpressionD;
...
/* starting wrapper class:    nsIDOMNSXPathExpression */
/**
 * Interface for Mozilla specific XPathExpression functions.
 */
class nsIDOMNSXPathExpressionD : public nsISupportsD {

  static const nsIID IID = NS_IDOMNSXPATHEXPRESSION_IID;


  alias nsIDOMNSXPathExpression InnerType;

  this(nsIDOMNSXPathExpression intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMNSXPathExpression opCast() {
    return inner;
  }

  void opAssign(nsIDOMNSXPathExpression value) {
    inner = value;
  }
  ...
  nsISupportsD  EvaluateWithContext(nsIDOMNodeD contextNode, PRUint32 contextPosition, PRUint32 contextSize, PRUint16 type, nsISupportsD result){
    nsISupports _retval;
    nsresult __result = inner.EvaluateWithContext(contextNode ? cast(nsIDOMNode)contextNode : null, contextPosition, contextSize, type, result ? cast(nsISupports)result : null, &_retval);
    CheckException(__result);
    return new nsISupportsD(_retval);
  }

private:
  nsIDOMNSXPathExpression inner;
