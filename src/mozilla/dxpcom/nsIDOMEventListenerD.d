...
module mozilla.dxpcom.nsIDOMEventListenerD;
...
/* starting wrapper class:    nsIDOMEventListener */
class nsIDOMEventListenerD : public nsISupportsD {

  static const nsIID IID = NS_IDOMEVENTLISTENER_IID;


  alias nsIDOMEventListener InnerType;

  this(nsIDOMEventListener intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMEventListener opCast() {
    return inner;
  }

  void opAssign(nsIDOMEventListener value) {
    inner = value;
  }
...
/**
  ...
  void HandleEvent(nsIDOMEventD event){
    nsresult __result = inner.HandleEvent(event ? cast(nsIDOMEvent)event : null);
