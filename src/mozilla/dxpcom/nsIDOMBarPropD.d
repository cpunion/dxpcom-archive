...
module mozilla.dxpcom.nsIDOMBarPropD;
...
/* starting wrapper class:    nsIDOMBarProp */
class nsIDOMBarPropD : public nsISupportsD {

  static const nsIID IID = NS_IDOMBARPROP_IID;


  alias nsIDOMBarProp InnerType;

  this(nsIDOMBarProp intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMBarProp opCast() {
    return inner;
  }

  void opAssign(nsIDOMBarProp value) {
    inner = value;
  }
 ...
 */
  ...
  PRBool Visible(){
    PRBool value;
    nsresult __result = inner.GetVisible(&value);
    CheckException(__result);
    return value;
  }
  void Visible(PRBool aVisible){
    nsresult __result = inner.SetVisible(aVisible);
    CheckException(__result);
  }
