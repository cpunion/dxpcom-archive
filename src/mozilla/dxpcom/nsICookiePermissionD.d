...
module mozilla.dxpcom.nsICookiePermissionD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsICookiePermission;


public import mozilla.dxpcom.nsICookiePermissionD;
...
/* starting wrapper class:    nsICookiePermission */
/**
 * An interface to test for cookie permissions
 */
class nsICookiePermissionD : public nsISupportsD {

  static const nsIID IID = NS_ICOOKIEPERMISSION_IID;


  alias nsICookiePermission InnerType;

  this(nsICookiePermission intr){
    super(intr);
    this.inner = intr;
  }

  nsICookiePermission opCast() {
    return inner;
  }

  void opAssign(nsICookiePermission value) {
    inner = value;
  }
  ...
  void SetAccess(nsIURID aURI, nsCookieAccess aAccess){
    nsresult __result = inner.SetAccess(aURI ? cast(nsIURI)aURI : null, aAccess);
  ...
  nsCookieAccess CanAccess(nsIURID aURI, nsIURID aFirstURI, nsIChannelD aChannel){
    nsCookieAccess _retval;
    nsresult __result = inner.CanAccess(aURI ? cast(nsIURI)aURI : null, aFirstURI ? cast(nsIURI)aFirstURI : null, aChannel ? cast(nsIChannel)aChannel : null, &_retval);
    CheckException(__result);
    return _retval;
  }
  ...
  PRBool CanSetCookie(nsIURID aURI, nsIChannelD aChannel, nsICookie2D aCookie, out PRBool aIsSession, out PRInt64 aExpiry){
    PRBool _retval;
    nsresult __result = inner.CanSetCookie(aURI ? cast(nsIURI)aURI : null, aChannel ? cast(nsIChannel)aChannel : null, aCookie ? cast(nsICookie2)aCookie : null, &aIsSession, &aExpiry, &_retval);
    CheckException(__result);
    return _retval;
  }

private:
  nsICookiePermission inner;
