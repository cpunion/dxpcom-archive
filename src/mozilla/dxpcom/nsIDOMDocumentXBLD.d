...
module mozilla.dxpcom.nsIDOMDocumentXBLD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsIDOMDocumentXBL;


public import mozilla.dxpcom.nsIDOMDocumentXBLD;
...
/* starting wrapper class:    nsIDOMDocumentXBL */
class nsIDOMDocumentXBLD : public nsISupportsD {

  static const nsIID IID = NS_IDOMDOCUMENTXBL_IID;


  alias nsIDOMDocumentXBL InnerType;

  this(nsIDOMDocumentXBL intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMDocumentXBL opCast() {
    return inner;
  }

  void opAssign(nsIDOMDocumentXBL value) {
    inner = value;
  }

  /* nsIDOMNodeList getAnonymousNodes (in nsIDOMElement elt); */
  nsIDOMNodeListD  GetAnonymousNodes(nsIDOMElementD elt){
    nsIDOMNodeList _retval;
    nsresult __result = inner.GetAnonymousNodes(elt ? cast(nsIDOMElement)elt : null, &_retval);
  ...
  nsIDOMElementD  GetAnonymousElementByAttribute(nsIDOMElementD elt, wchar[] attrName, wchar[] attrValue){
    scope auto _attrName = new AString(attrName);
    scope auto _attrValue = new AString(attrValue);
    nsIDOMElement _retval;
    nsresult __result = inner.GetAnonymousElementByAttribute(elt ? cast(nsIDOMElement)elt : null, cast(nsAString*)_attrName, cast(nsAString*)_attrValue, &_retval);
    CheckException(__result);
    return new nsIDOMElementD(_retval);
  }

  /* void addBinding (in nsIDOMElement elt, in DOMString bindingURL); */
  void AddBinding(nsIDOMElementD elt, wchar[] bindingURL){
    scope auto _bindingURL = new AString(bindingURL);
    nsresult __result = inner.AddBinding(elt ? cast(nsIDOMElement)elt : null, cast(nsAString*)_bindingURL);
    CheckException(__result);
  }

  /* void removeBinding (in nsIDOMElement elt, in DOMString bindingURL); */
  void RemoveBinding(nsIDOMElementD elt, wchar[] bindingURL){
    scope auto _bindingURL = new AString(bindingURL);
    nsresult __result = inner.RemoveBinding(elt ? cast(nsIDOMElement)elt : null, cast(nsAString*)_bindingURL);
    CheckException(__result);
  }

  /* nsIDOMElement getBindingParent (in nsIDOMNode node); */
  nsIDOMElementD  GetBindingParent(nsIDOMNodeD node){
    nsIDOMElement _retval;
    nsresult __result = inner.GetBindingParent(node ? cast(nsIDOMNode)node : null, &_retval);
    CheckException(__result);
    return new nsIDOMElementD(_retval);
  }

  /* nsIDOMDocument loadBindingDocument (in DOMString documentURL); */
  nsIDOMDocumentD  LoadBindingDocument(wchar[] documentURL){
    scope auto _documentURL = new AString(documentURL);
    nsIDOMDocument _retval;
    nsresult __result = inner.LoadBindingDocument(cast(nsAString*)_documentURL, &_retval);
    CheckException(__result);
    return new nsIDOMDocumentD(_retval);
  }

private:
  nsIDOMDocumentXBL inner;
