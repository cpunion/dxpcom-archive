...
module mozilla.dxpcom.nsIDOMPkcs11D;
...
/* starting wrapper class:    nsIDOMPkcs11 */
class nsIDOMPkcs11D : public nsISupportsD {

  static const nsIID IID = NS_IDOMPKCS11_IID;


  alias nsIDOMPkcs11 InnerType;

  this(nsIDOMPkcs11 intr){
    super(intr);
    this.inner = intr;
  }
  ...
  void opAssign(nsIDOMPkcs11 value) {
    inner = value;
  }

  /* long deletemodule (in DOMString moduleName); */
  PRInt32 Deletemodule(wchar[] moduleName){
    scope auto _moduleName = new AString(moduleName);
    PRInt32 _retval;
    nsresult __result = inner.Deletemodule(cast(nsAString*)_moduleName, &_retval);
    CheckException(__result);
    return _retval;
  }

  /* long addmodule (in DOMString moduleName, in DOMString libraryFullPath, in long cryptoMechanismFlags, in long cipherFlags); */
  PRInt32 Addmodule(wchar[] moduleName, wchar[] libraryFullPath, PRInt32 cryptoMechanismFlags, PRInt32 cipherFlags){
    scope auto _moduleName = new AString(moduleName);
    scope auto _libraryFullPath = new AString(libraryFullPath);
    PRInt32 _retval;
    nsresult __result = inner.Addmodule(cast(nsAString*)_moduleName, cast(nsAString*)_libraryFullPath, cryptoMechanismFlags, cipherFlags, &_retval);
    CheckException(__result);
    return _retval;
  }
