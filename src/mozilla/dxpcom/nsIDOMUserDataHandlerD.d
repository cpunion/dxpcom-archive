...
module mozilla.dxpcom.nsIDOMUserDataHandlerD;
...
/* starting wrapper class:    nsIDOMUserDataHandler */
class nsIDOMUserDataHandlerD {

  static const nsIID IID = NS_IDOMUSERDATAHANDLER_IID;


  alias nsIDOMUserDataHandler InnerType;

  this(nsIDOMUserDataHandler intr){
    this.inner = intr;
  }

  nsIDOMUserDataHandler opCast() {
    return inner;
  }

  void opAssign(nsIDOMUserDataHandler value) {
    inner = value;
  }
  ...
  void Handle(PRUint16 operation, wchar[] key, nsIVariantD data, nsIDOMNodeD src, nsIDOMNodeD dst){
    scope auto _key = new AString(key);
    nsresult __result = inner.Handle(operation, cast(nsAString*)_key, data ? cast(nsIVariant)data : null, src ? cast(nsIDOMNode)src : null, dst ? cast(nsIDOMNode)dst : null);
    CheckException(__result);
  }

private:
  nsIDOMUserDataHandler inner;
