...
module mozilla.dxpcom.nsIDOMPluginArrayD;
...
/* starting wrapper class:    nsIDOMPluginArray */
class nsIDOMPluginArrayD : public nsISupportsD {

  static const nsIID IID = NS_IDOMPLUGINARRAY_IID;


  alias nsIDOMPluginArray InnerType;

  this(nsIDOMPluginArray intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMPluginArray opCast() {
    return inner;
  }

  void opAssign(nsIDOMPluginArray value) {
    inner = value;
  }
  ...
  nsIDOMPluginD  Item(PRUint32 index){
    nsIDOMPlugin _retval;
    nsresult __result = inner.Item(index, &_retval);
    CheckException(__result);
    return new nsIDOMPluginD(_retval);
  }

  /* nsIDOMPlugin namedItem (in DOMString name); */
  nsIDOMPluginD  NamedItem(wchar[] name){
  ...
  void Refresh(PRBool reloadDocuments){
    nsresult __result = inner.Refresh(reloadDocuments);
    CheckException(__result);
  }

private:
  nsIDOMPluginArray inner;

}


/* starting wrapper class:    nsIDOMJSPluginArray */
class nsIDOMJSPluginArrayD : public nsISupportsD {

  static const nsIID IID = NS_IDOMJSPLUGINARRAY_IID;


  alias nsIDOMJSPluginArray InnerType;

  this(nsIDOMJSPluginArray intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMJSPluginArray opCast() {
    return inner;
  }

  void opAssign(nsIDOMJSPluginArray value) {
    inner = value;
  }
