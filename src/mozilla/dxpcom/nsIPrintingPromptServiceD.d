...
module mozilla.dxpcom.nsIPrintingPromptServiceD;

public import mozilla.dxpcom.nsrootidlD;


public import mozilla.xpcom.nsIPrintingPromptService;


public import mozilla.dxpcom.nsIPrintingPromptServiceD;
...
/* starting wrapper class:    nsIPrintingPromptService */
class nsIPrintingPromptServiceD : public nsISupportsD {

  static const nsIID IID = NS_IPRINTINGPROMPTSERVICE_IID;


  alias nsIPrintingPromptService InnerType;

  this(nsIPrintingPromptService intr){
    super(intr);
    this.inner = intr;
  }

  nsIPrintingPromptService opCast() {
    return inner;
  }

  void opAssign(nsIPrintingPromptService value) {
    inner = value;
  }
...
/**
  ...
  void ShowPrintDialog(nsIDOMWindowD parent, nsIWebBrowserPrintD webBrowserPrint, nsIPrintSettingsD printSettings){
    nsresult __result = inner.ShowPrintDialog(parent ? cast(nsIDOMWindow)parent : null, webBrowserPrint ? cast(nsIWebBrowserPrint)webBrowserPrint : null, printSettings ? cast(nsIPrintSettings)printSettings : null);
  ...
  void ShowProgress(nsIDOMWindowD parent, nsIWebBrowserPrintD webBrowserPrint, nsIPrintSettingsD printSettings, nsIObserverD openDialogObserver, PRBool isForPrinting, out nsIWebProgressListenerD webProgressListener, out nsIPrintProgressParamsD printProgressParams, out PRBool notifyOnOpen){
    nsIWebProgressListener _webProgressListener;
    nsIPrintProgressParams _printProgressParams;
    nsresult __result = inner.ShowProgress(parent ? cast(nsIDOMWindow)parent : null, webBrowserPrint ? cast(nsIWebBrowserPrint)webBrowserPrint : null, printSettings ? cast(nsIPrintSettings)printSettings : null, openDialogObserver ? cast(nsIObserver)openDialogObserver : null, isForPrinting, &_webProgressListener, &_printProgressParams, &notifyOnOpen);
    CheckException(__result);
  ...
  void ShowPageSetup(nsIDOMWindowD parent, nsIPrintSettingsD printSettings, nsIObserverD aObs){
    nsresult __result = inner.ShowPageSetup(parent ? cast(nsIDOMWindow)parent : null, printSettings ? cast(nsIPrintSettings)printSettings : null, aObs ? cast(nsIObserver)aObs : null);
    CheckException(__result);
  }
  ...
  void ShowPrinterProperties(nsIDOMWindowD parent, PRUnichar*printerName, nsIPrintSettingsD printSettings){
    nsresult __result = inner.ShowPrinterProperties(parent ? cast(nsIDOMWindow)parent : null, printerName, printSettings ? cast(nsIPrintSettings)printSettings : null);
    CheckException(__result);
  }

private:
  nsIPrintingPromptService inner;
