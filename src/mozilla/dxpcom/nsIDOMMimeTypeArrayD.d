...
module mozilla.dxpcom.nsIDOMMimeTypeArrayD;
...
/* starting wrapper class:    nsIDOMMimeTypeArray */
class nsIDOMMimeTypeArrayD : public nsISupportsD {

  static const nsIID IID = NS_IDOMMIMETYPEARRAY_IID;


  alias nsIDOMMimeTypeArray InnerType;

  this(nsIDOMMimeTypeArray intr){
    super(intr);
    this.inner = intr;
  }

  nsIDOMMimeTypeArray opCast() {
    return inner;
  }

  void opAssign(nsIDOMMimeTypeArray value) {
    inner = value;
  }
  ...
  nsIDOMMimeTypeD  Item(PRUint32 index){
    nsIDOMMimeType _retval;
    nsresult __result = inner.Item(index, &_retval);
    CheckException(__result);
    return new nsIDOMMimeTypeD(_retval);
  }

  /* nsIDOMMimeType namedItem (in DOMString name); */
  nsIDOMMimeTypeD  NamedItem(wchar[] name){
