...
module mozilla.dxpcom.nsIMsgProgressD;
...
/* starting wrapper class:    nsIMsgProgress */
class nsIMsgProgressD : public nsIWebProgressListenerD {

  static const nsIID IID = NS_IMSGPROGRESS_IID;


  alias nsIMsgProgress InnerType;

  this(nsIMsgProgress intr){
    super(intr);
    this.inner = intr;
  }

  nsIMsgProgress opCast() {
    return inner;
  }

  void opAssign(nsIMsgProgress value) {
    inner = value;
  }
  ...
  void OpenProgressDialog(nsIDOMWindowInternalD parent, nsIMsgWindowD aMsgWindow, char*dialogURL, nsISupportsD parameters){
    nsresult __result = inner.OpenProgressDialog(parent ? cast(nsIDOMWindowInternal)parent : null, aMsgWindow ? cast(nsIMsgWindow)aMsgWindow : null, dialogURL, parameters ? cast(nsISupports)parameters : null);
