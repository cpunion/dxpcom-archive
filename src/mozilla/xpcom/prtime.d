/* Converted to D from prtime.h by htod */
module mozilla.xpcom.prtime;
...
//C     #ifndef prtime_h___
//C     #define prtime_h___
...
//C     #define PR_MSEC_PER_SEC		1000UL
//C     #define PR_USEC_PER_SEC		1000000UL
const PR_MSEC_PER_SEC = 1000U;
//C     #define PR_NSEC_PER_SEC		1000000000UL
const PR_USEC_PER_SEC = 1000000U;
//C     #define PR_USEC_PER_MSEC	1000UL
const PR_NSEC_PER_SEC = 1000000000U;
//C     #define PR_NSEC_PER_MSEC	1000000UL
...
//C     typedef struct PRTimeParameters {
//C         PRInt32 tp_gmt_offset;     /* the offset from GMT in seconds */
//C         PRInt32 tp_dst_offset;     /* contribution of DST in seconds */
//C     } PRTimeParameters;
...
//C     typedef struct PRExplodedTime {
//C         PRInt32 tm_usec;		    /* microseconds past tm_sec (0-99999)  */
//C         PRInt32 tm_sec;             
/* seconds past tm_min (0-61, accomodating
                                   up to two leap seconds) */
	
//C         PRInt32 tm_min;             /* minutes past tm_hour (0-59) */
//C         PRInt32 tm_hour;            /* hours past tm_day (0-23) */
//C         PRInt32 tm_mday;            
/* days past tm_mon (1-31, note that it
				                starts from 1) */
//C         PRInt32 tm_month;           /* months past tm_year (0-11, Jan = 0) */
//C         PRInt16 tm_year;            
...
//C         PRTimeParameters tm_params;  /* time parameters used by conversion */
//C     } PRExplodedTime;
...
//C     typedef PRTimeParameters (PR_CALLBACK *PRTimeParamFn)(const PRExplodedTime *gmt);
...
//C     PRTime __pascal __export __loadds
//C     #else
//C     NSPR_API(PRTime) 
//C     #endif
//C     PR_Now(void);
...
//C     NSPR_API(void) PR_ExplodeTime(
//C         PRTime usecs, PRTimeParamFn params, PRExplodedTime *exploded);
...
//C     NSPR_API(PRTime) 
//C     #endif
//C     PR_ImplodeTime(const PRExplodedTime *exploded);
...
//C     NSPR_API(void) PR_NormalizeTime(
//C         PRExplodedTime *exploded, PRTimeParamFn params);
...
//C     NSPR_API(PRTimeParameters) PR_LocalTimeParameters(const PRExplodedTime *gmt);
PRTimeParameters  PR_LocalTimeParameters(PRExplodedTime *gmt);

/* Time parameters that represent Greenwich Mean Time */
//C     NSPR_API(PRTimeParameters) PR_GMTParameters(const PRExplodedTime *gmt);
 ...
 */
//C     NSPR_API(PRTimeParameters) PR_USPacificTimeParameters(const PRExplodedTime *gmt);
...
//C     NSPR_API(PRStatus) PR_ParseTimeString (
//C     	const char *string,
//C     	PRBool default_to_gmt,
//C     	PRTime *result);
...
//C     NSPR_API(PRUint32) PR_FormatTime(char *buf, int buflen, const char *fmt, 
//C                                                const PRExplodedTime *tm);
 ...
 */
//C     NSPR_API(PRUint32)
//C     PR_FormatTimeUSEnglish( char* buf, PRUint32 bufSize,
//C                             const char* format, const PRExplodedTime* tm );
